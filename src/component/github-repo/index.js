  ...
  async getAllRepos(
    ...
    try {
      const response = await graphql(`
        query {
          viewer { 
            repositories(privacy:PUBLIC first:100 ${endCursor ? 'after:"' + endCursor + '"' : ''}){
      ...
      const { pageInfo, totalCount, totalDiskUsage } = response.data.data.viewer.repositories
      repositories.totalCount = totalCount
      repositories.totalDiskUsage = totalDiskUsage
      repositories.nodes = repositories.nodes.concat(response.data.data.viewer.repositories.nodes)
  ...
  async getContributeRepos(
    ...
    try {
      const response = await graphql(`
        query{
          viewer{
            contributedRepositories(first:100 ${endCursor ? 'after:"' + endCursor + '"' : ''}){
              totalCount
              pageInfo{
      ...
      repositories.totalCount = totalCount
      repositories.nodes = repositories.nodes.concat(contributedRepositories.nodes)
    ...
    return (
      <Spin spinning={false}>
        ...
        >
          ...
          <Col span={8}>
            <p>
              ...
              {repositories.map(repo => (repo.forks ? repo.forks.totalCount : 0)).reduce((a, b) => a + b, 0) || 0}
            </p>
            <p>收获Fork数</p>
          </Col>
          <Col
        ...
        >
          ...
          >
            <p>
              ...
              {(() => {
                const sortByStar = sortBy(repositories, repo => -(repo.stargazers ? repo.stargazers.totalCount : 0))
                const mostStarRepo = sortByStar[0]
                return mostStarRepo ? (
                  <Tooltip title={`Star ${mostStarRepo.stargazers ? mostStarRepo.stargazers.totalCount : 0}`}>
                    {mostStarRepo.name}
                  </Tooltip>
                ) : (
                  ''
          ...
          <Col span={12}>
            {(() => {
              const sortByTime = sortBy(repositories, repo => -(new Date(repo.updatedAt) - new Date(repo.createdAt)))
        ...
        <Row style={{ padding: '2rem 0' }}>
          ...
          <Col md={12} sm={24} xs={24}>
            {(() => {
              let repos = sortBy(repositories || [], repo => -(repo.stargazers ? repo.stargazers.totalCount : 0))
              ...
              return ReactChart ? (
                <ReactChart
                  ...
                  data={{
                    ...
                    datasets: [
                      {
                        data: repos.map(repo => (repo.stargazers ? repo.stargazers.totalCount : 0)),
        ...
        <Row>
          <Col xs={{ span: 24 }} sm={{ span: 24 }} md={{ span: 16, offset: 4 }} lg={{ span: 12, offset: 6 }}>
            {(() => {
        ...
        >
          <Col span={24}>
            <p>参与贡献了{contributedRepositories ? contributedRepositories.totalCount : 0}个项目:</p>
            {contributedRepositories ? (
              contributedRepositories.nodes.map(v => {
                return (
                  <Tag style={{ marginTop: '0.5rem', marginBottom: '0.5rem' }} key={v.nameWithOwner || v.name}>
