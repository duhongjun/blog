...
query {
  ...
  async getFollowers(page, per_page) {
    let followers = []
    try {
      const { data, headers } = await github.get(`/users/${CONFIG.owner}/followers`, {
    ...
    return (
      <Spin spinning={!followers || !followers.length}>
        <Row>
          {followers.map(user => {
            return (
              <Col className="text-center" md={4} sm={6} xs={6} key={user.login}>
                <a href={user.url} target="_blank">
                  <img src={user.avatarUrl} style={{ width: '10rem', maxWidth: '100%' }} alt="" />
                  <br />
                  <sub>{user.login}</sub>
                </a>
              </Col>
