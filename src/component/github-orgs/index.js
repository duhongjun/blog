...
query{
  ...
  async componentWillReceiveProps(nextProp) {
    if (nextProp.orgs && nextProp.orgs.length) {
      ...
      current && !this.state.currentOrg && this.setState({ currentOrg: current.login, a: 22 })
    }
  }
  ...
  async statRepo(owner, repo) {
    let contributions = []
    try {
      const response = await github.get(`/repos/${owner}/${repo}/stats/contributors`)
      ...
      >
        ...
        <Col span={8}>
          <p>
            ...
            {(() => {
              const currentOrg = this.props.ORGS.find(org => org.login === this.state.currentOrg)
        ...
        >
          <p>
            ...
            {(() => {
              const currentOrg = this.props.ALL_ORG_REPOS[this.state.currentOrg]
              return currentOrg ? currentOrg.map(repo => repo.watchers_count).reduce((a, b) => a + b, 0) : 0
            })()}
          </p>
          <p>收获star数</p>
        </Col>
  ...
  render() {
    return (
      <Spin spinning={!this.props.ALL_ORG_REPOS}>
        ...
        <Tabs
          defaultActiveKey={this.props.ORGS && this.props.ORGS.length ? this.props.ORGS[0].login : ''}
          onChange={tab =>
            this.setState({
              currentOrg: tab
            })
            ...
            return (
              ...
              >
                ...
                <Row>
                  <Col span={24}>
                    {(() => {
                      let repos = sortBy(this.props.ALL_ORG_REPOS[org.login] || [], repo => -repo.watchers_count)
                      ...
                      return repos.map(repo => {
                        return (
                                ...
                                <p>
                                  ...
                                  {(() => {
                                    ...
                                    const myStat = stats.find(
                                      stat => stat && stat.author && stat.author.login === CONFIG.owner
                                    )
                                    if (myStat) {
                                      return (
                                        (myStat.contribution.changes / myStat.contribution.total * 100).toFixed(2) + '%'
                                      )
                                    } else {
                                      return '0%'
                                    }
                          ...
                          >
                            ...
                            >
                              <div className="org-contrib-row" style={styles.contributionBar}>
                                <div
