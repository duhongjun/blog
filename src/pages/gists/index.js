...
query{
  viewer{
    gists(first:${this.state.meta.per_page} ${endCursor ? 'after:"' + endCursor + '"' : ''}){
      totalCount
      nodes{
        name description id
      }
    ...
    return (
      <DocumentTitle title={['Gist List']}>
        <div className="toolbar-container">
          ...
          <Menu mode="inline" style={{ overflowY: 'auto', overflowX: 'hidden', borderRight: 0 }}>
            {this.props.GISTS.map(gist => {
              return (
                ...
                >
                  ...
                  >
                    <Octicon style={{ fontSize: '1.6rem', marginRight: '0.5rem' }} name="gist" mega />
                    {gist.description}
                  </NavLink>
                </Menu.Item>
              )
