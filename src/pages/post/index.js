...
class Post extends Component {
  ...
  async getPost(number) {
    let post = {}
    try {
      const { data } = await github.get(`/repos/${CONFIG.owner}/${CONFIG.repo}/issues/${number}`, {
        headers: {
  ...
  getShareMenu(post) {
    const shareMenu = [
      {
        title: '分享到新浪微博',
        url: `http://service.weibo.com/share/share.php?appkey=&title=${'分享: ' + post.title}&url=${
      ...
      },
      {
        ...
        url: `http://connect.qq.com/widget/shareqq/index.html?site=Axetroy's NeverLand&title=${'分享: ' +
          post.title}&summary=欢迎来到 Axetroy's NeverLand。&pics=&url=${window.location.href}`
      }
    ]
    return (
      <Menu>
    ...
    return (
      <DocumentTitle title={[post.title, '博客文章']}>
        <Spin spinning={!Object.keys(post).length}>
              ...
              >
                ...
                >
                  <Popover
                    ...
                    title={'在其他设备中扫描二维码'}
                    trigger="click"
                    content={<div className="text-center">{QRCode ? <QRCode value={window.location.href} /> : 'Loading...'}</div>}
                  >
                    <Icon type="qrcode" />
                  </Popover>
                </span>
          ...
          >
            <Comments type="issues" owner={CONFIG.owner} repo={CONFIG.repo} number={post.number} />
          </div>
        </Spin>
      </DocumentTitle>
    )
